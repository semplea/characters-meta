...
#								print str(fullTextClasses)
						...
						otherCatCount = sum([k[1] for k in categoriesBasedDisambiguation if k[0]!=bestCat[0]])
						foundDisambiguation = [bestCat[0], max(0, bestCatCount-otherCatCount)/len(categoriesBasedDisambiguation)]
						if (bestCatCount==0):
							print originalTerm
							print term
							print bestCat[0]
...
classes_local = {}
for root, dirs, files in os.walk("proximitywordclasses"):
    for file in files:
...
def obviousPredictor(word, indexesOfSentencesContainingWord, sentences, debug=False):
	...
	if (scoresSum>(2*len(scores))):
		minV = max(0, min(scores.values()))
		return [maxK, (maxV-minV)/scoresSum]		# we trust the result only if we saw enough samples, that is on average more than two by category
	else:
		return [u'unknown', 0]
...
#				if (sentence["tags"][wIdx]!='NAM'):
	...
	posThreshold = 0.45
	if (meanpos>0.45):
		return ['place', 1-abs(meanpos - (1-posThreshold)/2)/((1-posThreshold)/2)]
	else:
		return ['character', 1-abs(meanpos - posThreshold/2)/(posThreshold/2)]
...
'''
def structuralPredictor2(word, indexesOfSentencesContainingWord, sentences, debug=False):
	if (debug):
		print("***** Structural results 2 for "+word+" *****")
	scores = {u"place":0,u"character":0,u"other":0,u"unknown":0}
	predictions = {}
	matchedRules = {}
	for index in indexesOfSentencesContainingWord:
		...
		for wIdx, w in enumerate(sentence["words"]):
			if (w == word):
				s = [tag.split(':')[0] for tag in getSurroundings(sentence["tags"], wIdx)]
				skey = s[0]+s[1]+s[2]+s[3]
				for rulePred in structuralRules:
					if rulePred[0].match(skey):
						if (debug):
							storeCount(matchedRules, str(rulePred[0])+" --> "+rulePred[1])
						storeCount(predictions, rulePred[1])
	if (debug):
		for r, c in matchedRules.iteritems():
			print("Matched rule: "+r+" (x "+str(c)+")")
	maxV = 0
	maxK = ''
	scoresSum = 0
	for k,v in predictions.iteritems():
		scoresSum = scoresSum+v
		if (v>maxV):
			maxV = v
			maxK = k
	return [maxK, maxV/scoresSum if scoresSum>0 else 0]
...
#			print('Adjusted predictors weights: '+str(weights))
		...
		with open('results-'+bookfile.split("/")[-1], 'wb') as f:
