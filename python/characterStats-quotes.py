...
def obviousPredictor(word, indexesOfSentencesContainingWord, sentences, debug=False):
	...
	scores = {}
	predictingWords = []
	obviousChars = ['monsieur','madame','mademoiselle','monseigneur','m','m.','mme','mlle']
	obviousPlaces = ['pays','région','ville','village','château','rue','chemin','quai','mont','forêt','lac','mer']
...
def getQuotesPredictorThreshold(words, wsent, sentences, debug):
	...
	ratio = np.median(speakMentionsRatios)
	if (debug):
		print("***********************************************************")
...
def quotesPredictor(word, indexesOfSentencesContainingWord, sentences, quotesPredictorThreshold, debug=False):
	...
	for index in indexesOfSentencesContainingWord:
		...
		if ("PUN:cit" in s["tags"]):
			print("CIT: "+" ".join(sentences[index]["words"]));
			quotesCount = quotesCount+1
	if (quotesCount>0):
...
#			rel = (max(wsent[word])-min(wsent[word]))/len(sentences)
#			if (rel<(1/3)):
#				print(word+" :: "+'{:0.3f}'.format(rel)+' (IRRELEVANT?)')
...
def processBook(bookfile, mwsite, focus, benchmark, debug=False, verbose=False, graphs=False):
	...
	with open(bookfile) as f:
		...
		for word, wcount in sorted_ucw:
			...
			if (debug):
				print('-----------------------------------')
# TODO: tweak weights according to allpredictions results. For instance, remove predictors whose % deviate too much from the others
#		charsPlacesRatio = []
#		predictorRatioCounts = []
#
#		for pIdx in range(0,len(weights)-1):
