...
# sys.path.append(getScriptPath() + '/python/treetagger-python')
sys.path.append('/home/alexis/Documents/EPFL/MS3/Project/python/treetagger-python/')
from treetagger3 import TreeTagger
# TreeTagger = imp.load_source(
#               'TreeTagger',
#               '/home/alexis/Documents/EPFL/MS3/Project/python/treetagger-python/treetagger3.py')
...
stopwords = set(line.strip() for line in open(getScriptPath() + "/classifiersdata/stopwords.txt", 'r') if line.decode('utf-8') != '')
stopwords_pnouns = set(
	line.strip() for line in open(getScriptPath() + "/classifiersdata/stopwords_pnouns.txt", 'r') if line.decode('utf-8') != '')
...
WORD_FREQUENCE_THRESHOLD = 5  # Names that are mentioned less than n times in the whole book will be ignored
...
hunspellstemmer = hunspell.HunSpell(getScriptPath() + '/dictionaries/fr-toutesvariantes.dic',
									getScriptPath() + '/dictionaries/fr-toutesvariantes.aff')
...
onlineDisambiguationClasses = {
	"character": [
		"personnage", "personnalité", "prénom", "animal", "saint", "naissance", "décès", "peuple", "ethni", "patronym"],
	"place": [
		"lieu", "ville", "commune", "pays", "région", "territoire", "province", "toponym", "géographi",
		"géolocalisé", "maritime"],
...
onlineDisambiguationStopwords = ["wikip", "article", "littérature",
								 "littéraire"]
...
def onlineDisambiguation(site, term, originalTerm=None, debug=False, iter=1, checkedClasses=[]):
	...
	if cachedResult is True and not debug:
		return cachedResult
	else:
		if site:
			if iter < 5:
...
for root, dirs, files in os.walk(getScriptPath() + "/classifiersdata/proximitywordclasses"):
	for file in files:
		if file.endswith(".txt"):
			...
			classes_local[file.replace(".txt", "")] = [unicode(line.strip(), 'utf8') for line in wordsfile if
													   line[0] != b"#"]
...
def obviousPredictor(word, indexesOfSentencesContainingWord, sentences, debug=False):
	...
	scores = {}
	predictingWords = []
	obviousChars = ['m', 'm.', 'mr', 'monsieur', 'messieurs', 'mme', 'mrs', 'madame', 'mesdames', 'miss',
					'mademoiselle', 'mesdemoiselles', 'veuf', 'veuve', 'docteur', 'doctoresse', 'maître', 'maîtresse',
					'professeur', 'professeure', 'duc', 'duchesse', 'archiduc', 'archiduchesse', 'grand-duc',
					'grande-duchesse', 'marquis', 'marquise', 'comte', 'comtesse', 'vicomte', 'vicomtesse', 'baron',
					'baronne', 'seigneur', 'sieur', 'dame', 'écuyeur', 'messire', 'sir', 'lady', 'lord', 'émir',
					'émira', 'chérif', 'chérifa', 'cheikh', 'cheykha', 'bey', 'calife', 'hadjib', 'nizam', 'pervane',
					'sultan', 'vizir', 'râja', 'rani', 'maharadjah', 'maharajah', 'maharaja', 'malik', 'shah', 'chah',
					'padishah', 'khan', 'altesse', 'excellence', 'majesté', 'dom', 'don', 'père', 'mère', 'soeur',
					'frère', 'fils', 'fille', 'abbé', 'curé', 'révérend', 'inquisiteur', 'inquisitrice', 'évêque',
					'cardinal', 'monseigneur', 'messeigneurs', 'éminence', 'sainteté', 'pharaon', 'despote', 'magnat',
					'sire', 'pape', 'pontife', 'roi', 'reine', 'prince', 'princesse', 'empereur', 'impératrice',
					'infant', 'kronprinz', 'kaiser', 'aspirant', 'caporal', 'colonel', 'commandant', 'commandante',
					'lieutenant', 'maréchal', 'sergent', 'officier', 'sous-officier', 'soldat']
	obviousPlaces = ['pays', 'région', 'département', 'ville', 'village', 'cité', 'avenue', 'allée', 'boulevard', 'rue',
					 'chemin', 'quai', 'cathédrale', 'abbaye', 'église', 'chapelle', 'mont', 'colline', 'forêt', 'bois',
					 'océan', 'mer', 'lac', 'étang']
			...
			if w == word:
				if "VER:" in sentence["tags"][wIdx]:
					...
					if surroundings[1] == "PUN":  # noise detection (wrongly tokenized sentences).
						...
						scores[u"unknown"] -= 1.0
					if (surroundings[0] == "---" and surroundings[
						1] == "---"):
						# noise detection (wrongly tokenized sentences).
						# If this happens, needs to be compensated 2 times
...
def tokenizeAndStructure(text):
	taggedText = tt.tag(text.decode('utf-8'))
...
def bestChoice(_predictions, weights=[], debug=False):
	...
	if allAgree:
		...
		if (len(predClasses) == len(
				predictions)):  # we have exactly as many classes as predictions
			# (i.e. each predictor said something different)
...
def processBook(bookfile, mwsite, focus, benchmark, debug=False, verbose=False, graphs=False):
	...
	with open(bookfile) as f:
		...
		for i, raw_line in enumerate(f):
			line_split = unicode(raw_line, 'utf-8').split(u"\t")
		...
		for word, wcount in sorted_ucw:
			...
			if debug:
				...
				#     charsPlacesRatio.append((len([1 for wp in allPredictions
				#       if allPredictions[wp][pIdx][0]=='character']))/(len([1 for wp in allPredictions
				#       if allPredictions[wp][pIdx][0]=='place'])+1))
		...
		if len(benchmark) > 0:
			...
			unknown_words = []
			correct_predictors = {}
			ref_count = {}  # reference (number of words that should fall in each category, by predictor;
			# last idx=best choice)
			attr_count = {}  # attributions (number of words that fell in each category, by predictor;
			# last idx=best choice)
		...
		color_sequence = [
			"#000000", "#1CE6FF", "#FF34FF", "#FF4A46", "#008941", "#006FA6", "#A30059", "#FFDBE5",
			"#7A4900", "#0000A6", "#63FFAC", "#B79762", "#004D43", "#8FB0FF", "#997D87", "#5A0007",
			"#809693", "#FEFFE6", "#1B4400", "#4FC601", "#FFFF00", "#3B5DFF", "#4A3B53", "#FF2F80",
				...
				if not api:
					...
					for w1idx, w1 in enumerate(finalWordClasses['character']):
						...
						if len(xs) > 1:
							...
							# plt.plot(xs_sorted, ys_sorted, 'o', x_linspace, f2(x_linspace), '-',
							# lw=2, color=color_sequence[w1idx % len(color_sequence)], label=w1, markersize=8,
		...
		if len(benchmark) > 0:
			...
			benchStr = bookfile + "\t" + str() + "\t" + str(
				WORD_FREQUENCE_THRESHOLD)  # +"\t"+str(ucwtotcount)+"\t"+str(ucwtotunique)+"\t"+str(sorted_ucw[0][1])+"\t"+str(len(re.findall(r'\w+', fulltext)))
			ps = []
			rs = []
			for idx in precision_by_classes.keys():
...
try:
	opts, args = getopt.getopt(sys.argv[1:], "abcdfgsxw:v",
							   ["help", "benchmark", "graphs", "api", "file=", "focus=", "save",
								"mwclient=","mincount="])
